...
"<!DOCTYPE html>\r\n"
"<html><head><title>Network Programming HW1</title>\r\n"
"<style>body { background-color: #FFFF77 } </style></head>\r\n"
"<body><center><h1>405410038 NP_Hw1 </h1><br>\r\n"
  	 ...
  	 if(fd_client==-1){
         ...
         printf("Client Connection...\n ");
            if(!fork()){
                ...
                printf("%s\n",buf);
                if(!strncmp(buf, "GET /IMG_2120.jpg", 12)){
			fdimg = open("IMG_2120.jpg", O_RDONLY);
			sendfile(fd_client, fdimg,NULL,500000);
			close(fdimg);
		}
		else
